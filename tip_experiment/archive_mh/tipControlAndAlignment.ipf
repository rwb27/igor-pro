...
function tipAlign()
	...
	// Create waves to hold results
	make/o/n=(qMax,qMax) tipAlignGridScan_X			// Lock-in X data
	make/o/n=(qMax,qMax) tipAlignGridScan_Y			// Lock-in Y data
	make/o/n=(qMax,qMax) tipAlignGridScan_R			// Lock-in R data
	make/o/n=(qMax,qMax) tipAlignGridScan_theta		// Lock-in theta data
	make/o/n=(qMax,qMax,500) tipAlignGridScan_yPSD	// yPSD full waves
	make/o/n=(qMax,qMax) tipAlignGridScan_yPSDpp	// yPSD peak-to-peak
	make/o/n=0 tempPSD								// Temporary wave to hold PSD data

	
	// Initialize tektronix oscilloscope for communications
	...
	N=500
	oscStartUp()													// Initialize oscilloscope
	...
	setScale/P x, posB, gridStep, tipAlignGridScan_X, tipAlignGridScan_Y, tipAlignGridScan_R, tipAlignGridScan_theta, tipAlignGridScan_yPSD, tipAlignGridScan_yPSDpp
	setScale/P y, posC, gridStep, tipAlignGridScan_X, tipAlignGridScan_Y, tipAlignGridScan_R, tipAlignGridScan_theta, tipAlignGridScan_yPSD, tipAlignGridScan_yPSDpp
	...
	variable q = 0			// Counter for nested loop
	do
		do
			// Take measurements
			data = SR830_XY()
			...
			oscGetWave(2, "tempPSD")											// Get yPSD waveform from oscilloscope and store in tempPSD wave
			tipAlignGridScan_yPSDpp[qB][qC] = wavemax(tempPSD)-wavemin(tempPSD)	// Estimate peak-to-peak of PSD data
			// Store yPSD wave in 3D data wave
			q = 0
			do
				tipAlignGridScan_yPSD[qB][qC][q] = tempPSD[q]
				q = q+1			
			while(q<numpnts(tempPSD))
			doUpdate
...
function plotTipAlign(scanName)
	...
	appendimage/L=L5/B=B5 tipAlignGridScan_yPSDpp
	
	modifyGraph width=170.079,height=510.236	
	...
	modifyGraph freePos(B1)={0.8,kwFraction} 
	...
	modifyGraph axisEnab(L2)={0.6,0.8}
	modifyGraph tick(L2)=2
	modifyGraph freePos(B2)={0.6,kwFraction} 
	...
	modifyGraph axisEnab(L3)={0.4,0.6}
	modifyGraph tick(L3)=2
	modifyGraph freePos(B3)={0.4,kwFraction} 
	...
	modifyGraph axisEnab(L4)={0.2,0.4}
	modifyGraph tick(L4)=2
	modifyGraph freePos(B4)={0.2,kwFraction} 	
	modifyGraph tick(B4)=1,noLabel(B4)=2
	
	label L5 "yPSDpp"
	ModifyGraph lblPos(L5)=40
	modifyGraph freePos(L5)=0
	modifyGraph mirror(L5)=1
	modifyGraph axisEnab(L5)={0,0.2}
	modifyGraph tick(L5)=2
	modifyGraph freePos(B5)={0.0,kwFraction} 	
	modifyGraph tick(B5)=2,mirror(B5)=1
	
	modifyGraph standoff=0
	modifyGraph fSize=12
	
	modifyImage tipAlignGridScan_X ctab= {*,*,Geo32,0}
	ModifyImage tipAlignGridScan_X ctabAutoscale=1,lookup= $""	
	modifyImage tipAlignGridScan_Y ctab= {*,*,Geo32,0}
	ModifyImage tipAlignGridScan_Y ctabAutoscale=1,lookup= $""	
	modifyImage tipAlignGridScan_R ctab= {*,*,Geo32,0}
	ModifyImage tipAlignGridScan_R ctabAutoscale=1,lookup= $""	
	modifyImage tipAlignGridScan_theta ctab= {*,*,Geo32,0}
	ModifyImage tipAlignGridScan_theta ctabAutoscale=1,lookup= $""
	modifyImage tipAlignGridScan_yPSDpp ctab= {*,*,Geo32,0}
	ModifyImage tipAlignGridScan_yPSDpp ctabAutoscale=1,lookup= $""
...
function fitAlignmentData()
	...
	analyzeGridScan("tipAlignGridScan_yPSDpp")	// Fit yPSDpp-data
	duplicate W_coef, W_coef_psd				// Store theta-data best fit parameters
	duplicate W_sigma, W_sigma_psd			// Store theta-data error estimates
...
Function ButtonProc_57(ba) : ButtonControl
	STRUCT WMButtonAction &ba
...
function generateTipAlignReport(folderName)
	...
	make/o/n=(1,78) dayReport				// Make dayReport wave to hold variable data
	...
	do										// Loop through scan folders
		...
		wave wPSD = W_coef_psd
		wave wPSD_err = W_sigma_psd	
		...
		dayReport[q][64] = wPSD[0]
		dayReport[q][65] = wPSD[1]
		dayReport[q][66] = wPSD[2]
		dayReport[q][67] = wPSD[3]
		dayReport[q][68] = wPSD[4]
		dayReport[q][69] = wPSD[5]
		dayReport[q][70] = wPSD[6]
		
		dayReport[q][71] = wPSD_err[0]
		dayReport[q][72] = wPSD_err[1]
		dayReport[q][73] = wPSD_err[2]
		dayReport[q][74] = wPSD_err[3]
		dayReport[q][75] = wPSD_err[4]
		dayReport[q][76] = wPSD_err[5]
		dayReport[q][77] = wPSD_err[6]
		...
		redimension/n=(q+1,78) dayReport		// Increase size of wave
...
function generateDayReportKey() 					// Creates a text wave of parameter names
	...
	make/T/o/n=(1,78) dayReportKey				
		...
		dayReportKey[0][64] = "yPSD_z0"
		dayReportKey[0][65] = "yPSD_a0"
		dayReportKey[0][66] = "yPSD_x0"
		dayReportKey[0][67] = "yPSD_y0"
		dayReportKey[0][68] = "yPSD_sigx"
		dayReportKey[0][69] = "yPSD_sigy"
		dayReportKey[0][70] = "yPSD_corr"
		
		dayReportKey[0][71] = "yPSD_z0_err"
		dayReportKey[0][72] = "yPSD_a0_err"
		dayReportKey[0][73] = "yPSD_x0_err"
		dayReportKey[0][74] = "yPSD_y0_err"
		dayReportKey[0][75] = "yPSD_sigx_err"
		dayReportKey[0][76] = "yPSD_sigy_err"
		dayReportKey[0][77] = "yPSD_corr_err"
